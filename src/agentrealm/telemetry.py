...
environment. Enable by pointing any of these at a writable file:
    ...
    AGENTREALM_TELEMETRY=/path/to/telemetry.jsonl   # preferred
...
_SINK_ENV = ("AGENTREALM_TELEMETRY", "AGENTREALM_LLM_TRACE")
...
def llm_call_attributes(
    *,
    ...
    they exist (`gen_ai.*`) plus `agentrealm.*` for what OTel doesn't cover yet (the full rendered
    ...
    attrs: dict[str, Any] = {
        ...
        "agentrealm.request.system_prompt": system_prompt,
        "agentrealm.request.prompt": prompt,
        "agentrealm.request.tool_names": [
            t.get("function", {}).get("name") for t in tools if isinstance(t, dict)
        ],
        "agentrealm.request.tool_schemas": tools,
        "agentrealm.request.tool_choice": tool_choice,
        # the full raw output + any tool calls the model actually emitted
        "agentrealm.response.completion": completion,
        "agentrealm.response.tool_calls": tool_calls or [],
    }
    if realm_id is not None:
        attrs["agentrealm.realm.id"] = realm_id  # first-class filter: one realm's calls
    if agent_id is not None:
        attrs["agentrealm.agent.id"] = agent_id  # and one agent's calls within it
    if model_alias is not None:
        attrs["agentrealm.request.model_alias"] = model_alias  # as the provider encoded it
    ...
    if argv is not None:
        attrs["agentrealm.request.argv"] = argv
    if error is not None:
        attrs["error"] = True
        attrs["agentrealm.error.message"] = error
    return attrs
