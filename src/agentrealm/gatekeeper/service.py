...
from agentrealm.core.schema import Project, parse_duration
from agentrealm.core.settings import Settings, load_settings
from agentrealm.forge import DockerRuntime, Forge, RealmHandles, RealmtoolsConfig
from agentrealm.gatekeeper.appstate import active_provider, providers_config
from agentrealm.gatekeeper.runner import LiveSnapshot, Runner
from agentrealm.herald import BusProvision, Herald, HttpMatrixClient
from agentrealm.ledger import HttpLiteLLMClient, KeyStore, Ledger
from agentrealm.warden import ConcludeResult, TurnManager, Warden
...
def open_keystore() -> KeyStore:
    key = os.environ.get("AGENTREALM_KEYSTORE_KEY")
    if not key:
        raise ConfigError("AGENTREALM_KEYSTORE_KEY not set (Fernet key for the BYOK keystore)")
    path = Path.home() / ".agentrealm" / "keystore.json"
...
async def build_platform(settings: Settings | None = None) -> Platform:
    ...
    system_pw = os.environ.get("AGENTREALM_SYSTEM_PASSWORD")
    if not system_pw:
        raise ConfigError("AGENTREALM_SYSTEM_PASSWORD not set (must be stable across runs)")
    ...
    forge = Forge(
        ...
        flight_logs_dir=Path.home() / ".agentrealm" / "realms",
    )
