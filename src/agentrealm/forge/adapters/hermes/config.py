...
from agentrealm.forge.skills import skill_texts
...
class RealmtoolsCreds:
    ...
    url: str  # in-cluster, e.g. http://arealm-realmtools:9100/mcp
...
def _system_prompt(
    ...
    lines = [
        f'Operational context — AgentRealm. You are the agent "{agent.id}" ({matrix.user_id}),',
...
def render_hermes_home(
    ...
    *,
    ...
    default_soul = f"# {agent.name or agent.id}\n\nYou are an autonomous agent in AgentRealm."
