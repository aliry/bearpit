...
from agentrealm.core.interfaces import AgentHandle, RealmContext
from agentrealm.core.plugins import hooks_for
from agentrealm.core.schema import EgressTier, Project
from agentrealm.forge.adapters.hermes.adapter import HermesAdapter
from agentrealm.forge.adapters.hermes.config import MatrixCreds, RealmtoolsCreds
from agentrealm.forge.container import ContainerRuntime
from agentrealm.ledger import Ledger
from agentrealm.realmtools.tokens import mint_token
...
def orphan_containers(found: dict[str, str], active: Collection[str]) -> dict[str, str]:
    ...
    Shared by `Forge.reap_orphans` and the `arealm reap` command so the two cannot drift — they
...
class RealmtoolsConfig:
    ...
    url: str  # in-cluster MCP endpoint, e.g. http://arealm-realmtools:9100/mcp
    secret: str
...
class Forge:
    def __init__(
        ...
        *, realmtools: RealmtoolsConfig | None = None,
        ...
        # None = archival off (the platform wiring passes ~/.agentrealm/realms)
