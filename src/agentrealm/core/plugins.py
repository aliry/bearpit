...
an entry point in the `agentrealm.providers` group; its profiles are merged into the built-in table
...
PROVIDER_GROUP = "agentrealm.providers"
COMMAND_GROUP = "agentrealm.commands"
...
class ProviderPlugin(Protocol):
    """What an `agentrealm.providers` entry point must resolve to."""
...
def load_command_plugins(app: Any) -> None:
    """Let installed packages contribute `arealm` subcommands (`agentrealm.commands`).
    ...
    for ep in _entry_points(COMMAND_GROUP):
        try:
            register = ep.load()
            register(app)
        except Exception as exc:  # noqa: BLE001 - never let a plugin break `arealm`
