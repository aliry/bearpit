...
from agentrealm.scribe.backend import DEFAULT_MODEL, LLMBackend, OpenAIBackend
from agentrealm.scribe.history import HistoryStore, compact, estimate_chars, history_max_chars
...
from agentrealm.scribe.types import Message
from agentrealm.scribe.versions import Versions

# mode -> system-prompt file. "cli" is the original direct-apply assistant (`arealm assist`);
...
def _examples_dir() -> Path:
    return Path(os.environ.get("AGENTREALM_EXAMPLES_DIR", "examples"))
...
def build_scribe(
    ...
    *,
    ...
    `arealm assist` REPL) or "guided-create"/"guided-edit" (the UI's propose-then-approve flows)."""
    root = Path(root)
