"""Entry point for the `pit` CLI (M7)."""
...
from bearpit.core.jsonschema import write_schemas
from bearpit.core.plugins import load_command_plugins
from bearpit.scribe.backend import DEFAULT_MODEL
...
app = typer.Typer(
    name="pit",
    help="Bearpit — run realms of autonomous AI agents that collaborate or compete.",
...
@app.callback()
def callback() -> None:
    """Bearpit command-line interface."""
...
def reap(
    ...
    api: str = typer.Option(
        "http://127.0.0.1:8000", "--api", envvar="BEARPIT_API",
    ...
    from bearpit.forge import DockerRuntime, orphan_containers
    ...
    if not force:
        try:
            from bearpit.gatekeeper.auth import load_or_create_token
...
@app.command()
def version() -> None:
    """Print the Bearpit version."""
    from importlib.metadata import version as pkg_version

    typer.echo(f"bearpit {pkg_version('bearpit')}")
...
def serve(
    ...
    BEARPIT_KEYSTORE_KEY, BEARPIT_SYSTEM_PASSWORD, REALMTOOLS_SECRET).
    """
    import uvicorn

    from bearpit.gatekeeper.auth import load_or_create_token, token_path

    token = load_or_create_token()
    typer.secho(f"↑ Bearpit control panel → http://{host}:{port}/?token={token}",
                fg=typer.colors.CYAN)
    typer.secho(
    ...
    uvicorn.run("bearpit.gatekeeper.api:app", host=host, port=port)
...
def assist(
    ...
    data_root = Path(root) if root else Path.home() / ".bearpit" / "scribe"
...
@app.command()
def trace(
    path: str = typer.Argument("", help="Telemetry JSONL (default: $BEARPIT_TELEMETRY)"),
    ...
    and produced (completion + tool calls). Enable capture by pointing BEARPIT_TELEMETRY at a
    file before starting whatever component sits at the LLM chokepoint."""
    import json

    src = path or os.environ.get("BEARPIT_TELEMETRY") or os.environ.get("BEARPIT_LLM_TRACE")
    if not src or not Path(src).exists():
        typer.secho("No telemetry file. Set BEARPIT_TELEMETRY=<file>, then run a "
    ...
    def _text(a: dict[str, Any]) -> str:
        parts = (a.get("bearpit.request.system_prompt", ""),
                 a.get("bearpit.request.prompt", ""),
                 a.get("bearpit.response.completion", ""))
        return " ".join(str(p) for p in parts).lower()

    if realm:
        spans = [s for s in spans
                 if s.get("attributes", {}).get("bearpit.realm.id") == realm
                 or (not s.get("attributes", {}).get("bearpit.realm.id")
                     and realm.lower() in _text(s.get("attributes", {})))]
    if agent:
        spans = [s for s in spans
                 if s.get("attributes", {}).get("bearpit.agent.id") == agent]
    if grep:
    ...
    for s in shown:
        a = s.get("attributes", {})
        model = a.get("bearpit.request.model_alias") or a.get("gen_ai.request.model", "?")
        offered = a.get("bearpit.request.tool_names", [])
        called = a.get("bearpit.response.tool_calls", [])
        sysp = " ".join(a.get("bearpit.request.system_prompt", "").split())
        comp = " ".join(a.get("bearpit.response.completion", "").split())
        err = a.get("bearpit.error.message")
...
def keys_add(
    ...
    prompt, via any of (priority order): --api-key, piped stdin (e.g. `pbpaste | pit keys add
    …`), the BEARPIT_API_KEY env var. Falls back to a hidden prompt only on a bare tty. An
    explicit pipe wins over the env var, so a stale BEARPIT_API_KEY can't silently shadow it.
    Encrypted at rest with BEARPIT_KEYSTORE_KEY; never written to source or logs."""
    import os
    import sys

    from bearpit.gatekeeper.service import ConfigError, open_keystore

    if not api_key and not sys.stdin.isatty():
        api_key = sys.stdin.readline().strip()  # piped input wins over an ambient env var
    if not api_key:
        api_key = os.environ.get("BEARPIT_API_KEY", "")
    if not api_key:
...
def keys_list() -> None:
    ...
    if not handles:
        typer.echo("(no credentials — add one with: pit keys add <handle> --api-base <url>)")
...
async def _inject(realm_id: str, text: str) -> None:
    ...
    from bearpit.chronicle import EventKind
    from bearpit.core.settings import load_settings
    from bearpit.herald import Herald, HttpMatrixClient
    ...
    s = load_settings()
    sys_pw = os.environ.get("BEARPIT_SYSTEM_PASSWORD")
    if not sys_pw:
        _die("BEARPIT_SYSTEM_PASSWORD not set (needed to post as the system account)")
    herald = Herald(
...
async def _assist_repl(session: Any) -> None:  # pragma: no cover - interactive I/O
    ...
    from bearpit.scribe.loop import LoopEvent
...
def _confirm_provider(assume_yes: bool) -> bool:
    """Consent to run on a substitute provider (#47). Returns True once the choice is made.

    The stored provider can stop resolving without anyone editing anything — an ordinary
    `uv sync` prunes a separately-installed plugin, and the provider it contributed goes with it.
    The platform keeps working by falling back, but the substitute may be metered where the
    choice was flat-rate, so this asks rather than assumes."""
    from bearpit.gatekeeper.appstate import resolve_provider

    choice = resolve_provider(warn=False)   # this function is the warning
    if not choice.fell_back:
        return False
    typer.secho(f"⚠ model provider {choice.stored!r} is unavailable: {choice.reason}",
                fg=typer.colors.YELLOW)
    typer.secho(f"  This run would use {choice.name!r} instead, which may be metered.",
                fg=typer.colors.YELLOW)
    if not assume_yes and not typer.confirm("Continue?", default=False):
        raise typer.Exit(1)
    return True
...
# `bearpit.commands`). This runs after every built-in command is registered, so a plugin sees a
