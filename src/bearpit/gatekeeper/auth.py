...
The shape is the one Jupyter uses, because it solves the browser problem without a login form:
  ...
  * a token is generated once and kept in `~/.bearpit/api-token` (0600);
  * `pit serve` prints the URL with `?token=…` in it;
...
COOKIE_NAME = "bearpit_token"
QUERY_PARAM = "token"
HEADER = "authorization"
ENV_VAR = "BEARPIT_API_TOKEN"
...
def token_path() -> Path:
    return Path.home() / ".bearpit" / "api-token"


def load_or_create_token() -> str:
    """The API token, generated on first use.

    `BEARPIT_API_TOKEN` wins if set — that is how you pin a known value across restarts, or
