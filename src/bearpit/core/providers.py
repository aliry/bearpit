...
Settings (persisted in `~/.bearpit/platform.json`, see `gatekeeper.appstate`), and the resolver
...
from bearpit.core.plugins import plugin_profiles
from bearpit.core.schema import ModelCategory, ModelRef, Project
...
DEFAULT_PROVIDERS: dict[str, dict[str, Any]] = {
    ...
    OPENAI: {
        ...
        "setup_hint": "run `pit keys add openai-main --provider openai --api-key sk-...`",
        "categories": {
            "small":  {"model": "gpt-5.4-mini", "effort": None,
                       "input_cost_per_token": 7.5e-07, "output_cost_per_token": 4.5e-06,
                       "context_length": 400000},
    ...
    ANTHROPIC: {
        ...
        "setup_hint": (
            "run `pit keys add anthropic-main --provider anthropic --api-key sk-ant-...`"
        ),
        "categories": {
    ...
    OPENROUTER: {
        ...
        "setup_hint": (
            "run `pit keys add openrouter-main --provider openrouter "
